...
class ContactEnergyDailyGasSensor(BaseSensor, RestoreEntity):
    ...
    async def _async_perform_incremental_update(self, now: datetime) -> None:
        ...
        _LOGGER.debug("Performing incremental daily gas update")
        ...
        if response:
            ...
            _LOGGER.debug("Incremental daily gas update complete. Current daily m³: %.3f", self._daily_m3)
        else:
            _LOGGER.debug("No gas data available for incremental daily update")
